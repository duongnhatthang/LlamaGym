class ObsTranslator:
    ...
    def translate(self, state, nrow=4, ncol=4):
        ...
        res = f"You are at row {row}, column {col}."
        return res
...
class GameDescriber:
    ...
    def describe_game(self):
        return (
            "In the FrozenLake game, the player starts at the start position of the grid and tries to reach the"
    ...
    def describe_action(self):
        return (
            "For current position ('x', 'y'), the action means the player try to step into the next position. The possible actions are:"
